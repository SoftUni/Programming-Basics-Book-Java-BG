...
Внимание, **някои от тестовете е нормално да не минат**, защото съдържат грешно изписани числа (текст вместо число). С тях ще се справим по-късно, когато се запознаем с **`try-catch`** конструкцията.
