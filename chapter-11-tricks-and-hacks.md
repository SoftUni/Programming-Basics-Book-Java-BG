...
Програмният език Java е създаден от **Sun Microsystems** и те са тези, които определят най-добрите практики за писане. Трябва да знаете също така, че дори да не спазвате конвенциите, наложени от **Sun Microsystems**, кодът ви ще работи (стига да е написан правилно), но просто няма да бъде лесно разбираем. Това, разбира се, не е фатално на основно ниво, но колкото по-бързо свикнете да пишете качествен код, толкова по-добре.

За форматиране на кода от Sun Microsystems се препоръчва **къдравите скоби `{}`** да са: отварящата скоба да е на същия ред, а затварящата скоба да е точно под конструкцията, към която се отнасят, както е в примера по-долу.
...
Вижда се, че командата **`System.out.println(...)`** в примера е **4 празни полета навътре (един таб)**, което също се препоръчва от **Sun Microsystems**. Също така, ако дадена конструкция с къдрави скоби е един таб навътре, то затварящите **къдравите скоби `{}`** трябва да са в **началото на конструкцията**, както е в примера по-долу:
...
В тази подглава ще се фокусираме върху **общоприетите конвенции за именуване на проекти, файлове и променливи, наложени от Sun Microsystems.**
...
В тази подглава ще покажем как сами да си **направим собствен шаблон**. Ще разгледаме **как се прави code snippet** за **`scanner.nextLine();`**. Като за начало ще си създадем нов празен проект и ще отидем на [**File-> Settings -> Live Templates**], като избираме **+**, **LiveTemplate**, както е показано на снимката по-долу:

![](/assets/chapter-11-images/11_1_LiveTemplate.png)

В новия прозорец попълваме: Abbreviation, Description, Template text, маркираме: Reformat according to style и от Define избираме Java, след което натискаме **OK**. (показано на следващата снимка).
...
```
Първия аргумент на метода printf(…) е форматиращият стринг. В случая **%s** означава, да се постави Стринг **str**, на мястото на **%s**.
...
System.out.println(Math.рound(number));
...
System.out.println(Math.ceil(numberToCiling));
...
В случая използваме форматиращият стринг **`%.2f%n`**, което ще ограничи числовата променлива до 2 цифри след десетичния знак. Tрябва да имаме предвид, че числото пред буквата **`f`** означава до колко цифри след десетичния знак да е закръглено числото (т.е. може да е **``%.3f%n``** или **``%.5f%n``**).
