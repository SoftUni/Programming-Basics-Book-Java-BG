...
#### Насоки и подсказки
За изпълнението на задачата се налага да прибегнем до малка хитрост, ще използваме методи. В глава 10 ще се запознаете подробно с тях. Методът ни позволява да изпълнем един и същ код на повече от едно място в една програма. На този етап няма да разкриваме повече от концепцията на методите.

![](assets/chapter-6-images/01.Rectangle-of-10-x-10-stars-01.png)

Как работи примерът? Инициализира се управляваща променлива на **цикъла `i = 1`** и се увеличава на всяка итерация, докато е **по-малка или равна на 10**(проверката се извършва след всяко изпълнение на тялото на цикъла и след итерацията. Така кодът в тялото на цикълa се изпълнява точно **10 пъти**. Кодът в тялото на цикъла ще се извика за всеки ред, който ще изведе един ред от 10 (десет) звездички.
...
Вложените цикли представляват конструкция, при която в тялото на един цикъл (външен) се изпълнява друг цикъл (вътрешен). За всяко завъртане на външния цикъл, вътрешния се извърта целият. Това се случва по следният начин: при стартиране на програмата се изпълнява инициализацията на управляващата променлива във външния цикъл. След това се изпълнява вътрешният цикъл. Извършва инициализация на началната стойност на управляващата променлива във вложения (вътрешния) цикъл и се изпълнява кодът в тялото му. При достигане на зададената стойност за край на цикъла, програмата се връща една позиция нагоре и изпълнява предходния цикъл (външния цикъл). Инкрементира се управляващата променлива, проверява се дали условието е удовлетворено и отново се започва изпълнението на вложения цикъл. Това се повтаря докато променливата на външния цикъл не удовлетворява прекъсващото условие и надхвърля граничната стойност. 

![](assets/chapter-6-images/Nestedloops.png)

Нека разгледаме примера по-горе - след инициализацията на **външния** цикъл, започва да се изпълнява неговото **тяло**, което съдържа **втория (вложен или още вътрешен) цикъл**. Той извежда на един ред **`n`** на брой звездички. След като **вътрешният** цикъл **приключи** изпълнението си управлението на програмата се връща при първата итерация на външния, то след това **външният ще продължи**, т.е. ще отпечата един празен ред на конзолата. **След това** ще се извърши **инкрементиране**(управляващата променлива ще се увеличи с 1, в нашия случай) на променливата на **първия** цикъл и отново ще бъде изпълнен целия **втори** цикъл. Вътрешният цикъл ще се изпълни толкова пъти, колкото се изпълнява тялото на външния цикъл, в случaя **`n`** пъти.
