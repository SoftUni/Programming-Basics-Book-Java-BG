...
![](assets/chapter-10-images/_1_Simple_Method.png)

Този метод има задачата да отпечата заглавие, което представлява поредица от символа **`-`**. Поради тази причина името му е **`printHeader`**. Кръглите скоби **`( `** и **`)` винаги** следват името, независимо как сме именували метода. По-късно ще разгледаме как трябва да именуваме методите, с които работим, а за момента ще отбележим само, че е важно **името му да описва функцията**, която той изпълнява.
...
![](assets/chapter-10-images/_2_Main_Method.png)

Със следващия пример ще разгледаме задължителните елементи в декларацията на един метод.

![](assets/chapter-10-images/_3_GetSquare.png)

* **Модификатор** В случая модификатора за достъп е **`public`**, което означава, че всички останали класове и методи могат да извикват нашия метод. Модификаторите в Java са много, но основните са : **`public`**(видим от всички), **`private`**(видими само от настоящия клас), **`protected`**(видим от нашия клас и наследниците му).
...
* **Списък с параметри**. Декларира се между скобите **`(`** и **`)`**, които изписваме след името му. Тук изброяваме поредицата от **параметри**, които метода ще използва. Може да присъства **само един** параметър, **няколко** такива или да е **празен** списък. Ако няма параметри, то ще запишем само скобите **`()`**. В конкретния пример декларираме параметъра **`num`** от тип double.

При деклариране на методи е важно да спазваме **последователността** на основните му елементи - първо **модификатор за достъп**, следван от **тип на връщаната стойност**, след това **име на метода** и накрая **списък от параметри**, ограден с кръгли скоби **`()`**.
...
Извикването на метод представлява **стартирането на изпълнението на кода**, който се намира в **тялото на метода**. Това става като изпишем **името** му, последвано от кръглите скоби **`()`** и знака **`;`**. Ако методът ни изисква входни данни, то те се подават в  скобите **`()`**, като последователноста на фактическите параметрите трябва да съвпада с последователността на подадените при декларирането на метода. 

![](assets/chapter-10-images/_4_Invoke_Method.png)

Даден метод може да бъде извикан от **няколко места** в нашата програма. Единият начин е да бъде извикан от **главния метод**.

![](assets/chapter-10-images/_5_Invoke_Method_In_Main.png)

Метод може да бъде извикан и от **тялото на друг метод**, който **не** е главния метод на програмата ни.

![](assets/chapter-10-images/_6_Method_In_Method.png)
...
Първата ни стъпка е да създадем **`public void`** метод за принтиране на заглавната част от касовата бележка. Нека му дадем смислено име, което описва кратко и ясно задачата му, например **`printReceiptHeader`**. В тялото му ще напишем кода от примера по-долу.

![](assets/chapter-10-images/_7_Print_Receipt_1.png)
...
![](assets/chapter-10-images/_8_Print_Receipt_2.png)

Накрая ще **извикаме** метода **`printReceipt`** от тялото на метода **`main`**.

![](assets/chapter-10-images/_9_Print_Receipt_3.png)
...
![](assets/chapter-10-images/_10_Method_Parameters.png)

След това **извикваме** метода и му **подаваме конкретни стойности**:

![](assets/chapter-10-images/_11_Method_Parameters.png)
...
![](assets/chapter-10-images/_12_Method_Parameters_StudentExample.png)
...
![](assets/chapter-10-images/_13_Print_Sign_Method.png)
...
![](assets/chapter-10-images/_14_Print_Sign_In_Main.png)
...
![](assets/chapter-10-images/_15_Triangle_1.png)
...
![](assets/chapter-10-images/_16_Triangle_2.png)

След това разпечатваме средната линия.

![](assets/chapter-10-images/_17_Triangle_3.png)

Накрая разпечатваме долната част от триъгълника, като този път стъпката на цикъла намалява.

![](assets/chapter-10-images/_18_Triangle_4.png)
...
![](assets/chapter-10-images/_19_Square_1.png)
...
![](assets/chapter-10-images/_22_Void_Method.png)
...
![](assets/chapter-10-images/_23_Return_Method.png)
...
![](assets/chapter-10-images/_24_Return_Method_2.png)
...
След **`return`** операторa, в текущия блок, **не** трябва да има други редове код, тъй като тогава IntelliJ IDEA ще покаже предупреждение, съобщавайки ни, че е засякъл код, който **не може да бъде достъпен**. Въпреки това, **има случаи**, в които **`return`** може да бъде извикван от няколко места в метода, но само ако има **определени** входни условия.

![](assets/chapter-10-images/_25_CompareTo_Method.png)
...
![](assets/chapter-10-images/_26_TriangleArea.png)
...
![](assets/chapter-10-images/_27_Result_Of_TriangleArea.png)
...
![](assets/chapter-10-images/_28_Calculate_Power.png)
...
![](assets/chapter-10-images/_29_Print_Method.png)
...
![](assets/chapter-10-images/_30_Variant_Of_One_Method.png)
...
![](assets/chapter-10-images/_31_Wrong_Merhods.png)
...
![](assets/chapter-10-images/_34_Get_Max_String.png)
...
![](assets/chapter-10-images/_35_Get_Max_Main.png)
...
Когато наименуваме даден метод е препоръчително да използваме **смислени имена**. Тъй като всеки метод **отговаря** за някаква част от нашия проблем, то при наименуването му трябва да вземем предвид **неговата функция**, т.е. добра практика е **името да описва неговата цел**. Задължително е името да започва с **малка буква** и трябва да е съставено от глагол или от двойка – глагол и съществително име. Форматирането на името става, спазвайки **Lower Case Camel** конвенцията, т.е. **първата дума започва с малка буква, а всяка следваща започва с главна буква**. Кръглите скоби **`( `** и **`)`** винаги следват името му. 
...
![](assets/chapter-10-images/_36_Good_Practice.png)

Пример за **лошо** структуриран код:

![](assets/chapter-10-images/_37_Bad_Practice.png)
...
![](assets/chapter-10-images/_38_RepeatedString.png)
...
![](assets/chapter-10-images/_39_ShowSuccess.png)
...
![](assets/chapter-10-images/_40_ShowError.png)
